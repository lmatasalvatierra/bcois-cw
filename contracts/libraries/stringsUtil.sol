...
library stringsUtil {
    ...
    function uintToString(uint v) pure internal returns (string) {
        return bytes32ToString(uintToBytes(v));
    }

    function addressToString(address x) pure internal returns (string) {
    ...
    function uuidToString(bytes16 x) pure internal returns (string) {
        bytes memory s = new bytes(32);
        for (uint i = 0; i < 16; i++) {
            byte b = byte(uint8(uint(x) / (2**(8*(15 - i)))));
            byte hi = byte(uint8(b) / 16);
            byte lo = byte(uint8(b) - 16 * uint8(hi));
            s[2*i] = char(hi);
            s[2*i+1] = char(lo);
